-- Migration 009: Admin-controlled exclusion from assignment generation
--
-- Adds a local-only flag so admins can prevent a member from being
-- assigned to volunteers without touching their status in Planning Center.
--
-- The excluded_from_assignments column is intentionally absent from the
-- syncMembersDaily upsert payload, so it survives daily PCO syncs.

ALTER TABLE members
  ADD COLUMN excluded_from_assignments BOOLEAN NOT NULL DEFAULT false;

-- Allow admins to toggle this field from the client.
-- The service role (Edge Function) already bypasses RLS.
CREATE POLICY "Admins can update member exclusion"
  ON members FOR UPDATE
  USING (is_admin())
  WITH CHECK (is_admin());

-- Update assignment generation to skip excluded members.
...
BEGIN
  ...
  -- Assign ALL contactable, non-excluded members evenly across volunteers.
